...
import torch.optim
from read_data_validation import GBMValidation
import numpy as np
import torch
import copy
...
class SiameseSharedAndSpecificClassifier(nn.Module):
    def __init__(self, original_size=[495, 472, 457], view_size=[457, 256, 128], out_size=32, c_n_units=[64, 32]):
        ...
        # Classification
        self.classification_l1 = nn.Linear(out_size * 6, c_n_units[0])
...
def main():
    ...
    write_tool.write_gbm(train_size=train_size, test_size=test_size, validation_size=0.01, seed=43)
    train_data = GBMValidation(set_name='train')
    test_data = GBMValidation(set_name='test')

    # Build Model
    model = SiameseSharedAndSpecificClassifier(original_size=[1305, 12042, 534], view_size=[534, 256, 128], out_size=32, c_n_units=[64, 32])
    SharedSpecific1 = SharedAndSpecific1(view_size=[1305, 12042, 534], n_unit=[534, 534, 534], out_size=256)
    ...
    test_loader = torch.utils.data.DataLoader(dataset=test_data, batch_size=65, shuffle=True)
    for epoch in range(EPOCH):
        ...
        for iter, traindata in enumerate(train_loader):
            batch_view1, batch_view2, batch_view3, label = traindata
            ...
            scio.savemat('E:/Data/GBM_specific3.mat', {'GBM_specific3': batch_view3_specific.detach().cpu().numpy()})
            ...
            scio.savemat('E:/Data/GBM_shared3.mat', {'GBM_shared3': batch_view3_shared.detach().cpu().numpy()})
        ...
        for iter, testdata in enumerate(test_loader):
            ...
            if USE_GPU:
                test_view1_inputs, test_view2_inputs, test_view2_inputs, test_labels \
                    = Variable(test_view1_inputs.cuda()), \
                      Variable(test_view2_inputs.cuda()), \
                      Variable(test_view3_inputs.cuda()), test_labels.cuda()
            else:
